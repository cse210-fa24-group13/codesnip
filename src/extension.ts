...
export function activate(context: vscode.ExtensionContext) {
  ...
  let registerGlobalCIPSnippets: (() => vscode.Disposable) | undefined =
    undefined;
  ...
  if (
    !context.globalState.get(releaseChangelogId) &&
  ...
  let oldSnippetsPath: string =
    vscode.workspace.getConfiguration("snippets").get("snippetsLocation") ||
        ...
        .then((selection) => {
          switch (selection) {
            case migrateData:
              ...
              if (
                oldSnippets &&
                oldSnippets.children &&
                oldSnippets.children.length > 0
              ) {
                ...
                if (
                  dataAccess.hasNoChild() ||
                  !newSnippets.children ||
                  newSnippets.children.length !== oldSnippets.children.length
                ) {
                  vscode.window.showErrorMessage(
                    StringUtility.formatString(
                      Labels.snippetsDataNotRestored,
                      oldSnippetsPath
                    )
                  );
                } else {
                  fs.rename(
                    oldSnippetsPath,
                    `${oldSnippetsPath}_bak`,
                    (err) => {
                      if (err) {
                        vscode.window.showInformationMessage(
                          StringUtility.formatString(
                            Labels.snippetsDataRestoredButFileNotRenamed,
                            `${oldSnippetsPath}_bak`
                          )
                        );
                      } else {
  ...
  let snippetsExplorer = vscode.window.createTreeView("snippetsExplorer", {
    ...
    dragAndDropController:
      typeof vscode.DataTransferItem === "function"
        ? snippetsProvider
        : undefined,
  });
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      commands.CommandsConsts.miscRequestWSConfig,
      async (_) => {
  ...
  let triggerCharacter: any = vscode.workspace
    .getConfiguration(snippetsConfigKey)
    .get("triggerKey");
  if (!triggerCharacter) {
  ...
  let globalPrefix: any = vscode.workspace
    .getConfiguration(snippetsConfigKey)
    .get("globalPrefix");
  let camelize: any = vscode.workspace
    .getConfiguration(snippetsConfigKey)
    .get("camelize");
  ...
  for (const currentLanguage of allLanguages) {
    let disposable =
      currentLanguage.id === "**" ? globalCipDisposable : cipDisposable;
    const registerCIPSnippets = () =>
      (disposable = vscode.languages.registerCompletionItemProvider(
        currentLanguage.id === "**" // use pattern filter for non-language snippets
          ...
          : [
              { language: currentLanguage.id, scheme: "file" },
              { language: currentLanguage.id, scheme: "untitled" },
            ],
        {
            ...
            return candidates.map(
              (element) =>
                <vscode.CompletionItem>{
                  ...
                  detail: element.description?.replace(
                    "__(ws)",
                    " (snippet from workspace)"
                  ),
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      commands.CommandsConsts.commonCopySnippetToClipboard,
      async (snippet) =>
        handleCommand(async () => vscode.env.clipboard.writeText(snippet.value))
    )
  );

  context.subscriptions.push(
    vscode.commands.registerCommand(
      commands.CommandsConsts.commonOpenPage,
      ...
      if (message.command === "createSnippets") {
        try {
          ...
          if (session) {
            ...
            const tasks = Object.keys(gistInfo.data.files).map(
              async (fileName) => {
            ...
            vscode.window.showErrorMessage(
              "GitHub session could not be opened."
            );
          }
        } catch (error: any) {
          vscode.window.showErrorMessage(
          ...
          snippetsHtml += `
            <li class="card">
                <div class="bottom">
                    ...
                    <div class="row">
                        <button class="clipboard tooltip" onclick="copyToClipboard(\`${snippet.value}\`, \`${index}\`)">
                            <span class="tooltiptext" id="tooltip-${index}">Copied to Clipboard!</span>
                        ...
                        <button class="clipboard" onclick="copyToClipboard(\`${snippet.gistid}\`, \`${index}\`)">
                            <svg xmlns="http://www.w3.org/2000/svg" height="18" width="16.25"
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      ...
      (snippet) =>
        handleCommand(() => snippet && snippetsProvider.moveSnippetUp(snippet))
    )
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      ...
      (snippet) =>
        handleCommand(
          () => snippet && wsSnippetsProvider.moveSnippetUp(snippet)
        )
    )
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      ...
      (snippet) =>
        handleCommand(
          () => snippet && snippetsProvider.moveSnippetDown(snippet)
        )
    )
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      ...
      (snippet) =>
        handleCommand(
          () => snippet && wsSnippetsProvider.moveSnippetDown(snippet)
        )
    )
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      commands.CommandsConsts.globalSortSnippets,
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      commands.CommandsConsts.wsSortSnippets,
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      commands.CommandsConsts.globalSortAllSnippets,
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      commands.CommandsConsts.wsSortAllSnippets,
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("commonSnippetsCmd.refreshEntry", (_) =>
      refreshUI()
    )
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      commands.CommandsConsts.globalFixSnippets,
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      commands.CommandsConsts.wsFixSnippets,
    ...
    vscode.languages.registerDocumentDropEditProvider("*", {
      async provideDocumentDropEdits(
        ...
        const dataItem = dataTransfer.get(
          "application/vnd.code.tree.snippetsProvider"
        );
        if (!dataItem) {
          return;
        }
